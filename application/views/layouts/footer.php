        ...
        <div class="footer">
            <div class="pull-right">
                Contacto      <strong>administrador@acsoluciones.com</strong>
            </div>
            <div>
...
$(document).ready(function () {
    ...
    $(".btn-view-proveedor").on("click", function(){
        ...
        html += "<p><strong>Ruc: </strong>"+infoproveedor[2]+"</p>"
        html += "<p><strong>Dni: </strong>"+infoproveedor[3]+"</p>"
        html += "<p><strong>Direccion: </strong>"+infoproveedor[4]+"</p>"
        html += "<p><strong>Telefono: </strong>"+infoproveedor[5]+"</p>";
        $("#modal-default .modal-body").html(html);
    });
    ...
    $(".btn-view-categorias").on("click", function(){
        ...
        html += "<p><strong>Descrripcion :</strong>"+infocategoria[2]+"</p>"
        $("#modal-default .modal-body").html(html);
    });
    ...
    $(document).on("keyup","#tbcompras input.cantidades", function(){
        cantidad = $(this).val();
        precio = $(this).closest("tr").find("td:eq(2)").text();
        importe = cantidad * precio;
        $(this).closest("tr").find("td:eq(5)").children("p").text(importe.toFixed(2));
...
function agregar(id,codigo,nombre,precio,stock){
    html = "<tr>";
        html += "<td><input type='hidden' id='idproductos[]' name='idproductos[]' value='"+id+"'>"+codigo+"</td>";
        html += "<td>"+nombre+"</td>";
        html += "<td><input type='hidden' id='precios[]' name='precios[]' value='"+precio+"'>"+precio+"</td>";
        html += "<td>"+stock+"</td>";
        html += "<td><input type='text' id='cantidades[]' name='cantidades[]' value='1' class='cantidades'></td>";
        html += "<td><input type='hidden' name='importes[]' value='"+precio+"'><p>"+precio+"</p></td>";
        html += "<td><center><button type='button' class='btn btn-danger btn-remove-producto'><span class='fa fa-remove'></span></button></center></td>";
    html += "</tr>";
    $("#tbcompras").append(html);
};
function imprimir_compra(id){
    $.ajax({
        url: baseurl + "movimientos/compras/view",
        type:"POST",
        dataType:"html",
        data:{id:id},
        success:function(data){
...
function imprimir_compra_validar(id){
    $.ajax({
        url: baseurl + "movimientos/compras/view_validar",
        type:"POST",
        dataType:"html",
        data:{id:id},
        success:function(data){
            $("#modal-validar .modal-body").html(data);
        }
    });
}
</script>
